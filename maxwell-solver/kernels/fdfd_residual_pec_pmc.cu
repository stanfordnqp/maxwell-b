...
if (_Y == {{ dims[1] }}) {
    ...
    if ( pemc(3) == 2 ) {
        //PMC
        _Y = {{ dims[1]-1 }};
		pc_yz_factor[1] = -1.0; //this value does not matter
		pc_iy_Ex = -1;
		pc_iy_Ey = 0;
		pc_iy_Ez = -1;
	}
...
for (; _X < _x_end ; _X += _txx) {
    ...
    if (_in_global && _in_local) {
        {% if full_operator %}
        rx = Bx(0,0,0) - ((1.0 / (sqrt_sx1(_X) * sqrt_sy0(_Y) * sqrt_sz0(_Z))) *
                    (sy0(_Y) * (Hz_0[0] - Hz_0[s_yn])
                    - sz0(_Z) * (Hy_0[0] - Hy_0[s_zn])
                    - ex(0,0,0) * Ex_0[0]));
        ry = By(0,0,0) - ((1.0 / (sqrt_sx0(_X) * sqrt_sy1(_Y) * sqrt_sz0(_Z))) *
                    (sz0(_Z) * (Hx_0[0] - Hx_0[s_zn]) 
                    - sx0(_X) * (Hz_0[0] - Hz_n) 
                    - ey(0,0,0) * Ey_0[0]));
        rz = Bz(0,0,0) - ((1.0 / (sqrt_sx0(_X) * sqrt_sy0(_Y) * sqrt_sz1(_Z))) *
                    (sx0(_X) * (Hy_0[0] - Hy_n) 
