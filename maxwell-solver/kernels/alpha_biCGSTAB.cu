...
if (adj_dims) {
    // Load in p = r + beta * p.
    Ex_0[0] = bloch_phaseX_x * bloch_phaseYZ_x * (Rx(-1,0,0) + beta * (Px(-1,0,0) - omega * Vx(-1,0,0))) * 
                (sqrt_sx1(_X+xn) * sqrt_sy0(_Y) * sqrt_sz0(_Z));
    Ey_0[0] = bloch_phaseX_y * bloch_phaseYZ_y * (Ry(-1,0,0) + beta * (Py(-1,0,0) - omega * Vy(-1,0,0))) * 
                (sqrt_sx0(_X+xn) * sqrt_sy1(_Y) * sqrt_sz0(_Z));
    Ez_0[0] = bloch_phaseX_z * bloch_phaseYZ_z * (Rz(-1,0,0) + beta * (Pz(-1,0,0) - omega * Vz(-1,0,0))) * 
                (sqrt_sx0(_X+xn) * sqrt_sy0(_Y) * sqrt_sz1(_Z));

    // Ey_p = Ry(0,0,0) + beta * Ey(0,0,0);
    py_p = Ry(0,0,0) + beta * (Py(0,0,0) - omega * Vy(0,0,0));
    Ey_p = bloch_phaseYZ_y * (py_p) * (sqrt_sx0(_X) * sqrt_sy1(_Y) * sqrt_sz0(_Z));

    // Ez_p = Rz(0,0,0) + beta * Ez(0,0,0);
    pz_p = Rz(0,0,0) + beta * (Pz(0,0,0) - omega * Vz(0,0,0));
