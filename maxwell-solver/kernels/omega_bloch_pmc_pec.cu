...
if (adj_dims) {
    ...
    Ex_0[0] = bloch_phaseX_x * bloch_phaseYZ_x * pc_x_factor[0] * pc_yz_factor[0] *
                ...
                (  Rx(pc_ix_Ex, pc_iy_Ex, pc_iz_Ex) 
                 - alpha * Vx(pc_ix_Ex, pc_iy_Ex, pc_iz_Ex));
    Ey_0[0] = bloch_phaseX_y * bloch_phaseYZ_y * pc_x_factor[1] * pc_yz_factor[1] *
                ...
                (  Ry(pc_ix_Ey, pc_iy_Ey, pc_iz_Ey)
                 - alpha * Vy(pc_ix_Ey, pc_iy_Ey, pc_iz_Ey));
    Ez_0[0] = bloch_phaseX_z * bloch_phaseYZ_z * pc_x_factor[2] * pc_yz_factor[2] *
                ...
                (  Rz(pc_ix_Ez, pc_iy_Ez, pc_iz_Ez) 
                 - alpha * Vz(pc_ix_Ez, pc_iy_Ez, pc_iz_Ez));


    // Ey_p = Ry(0,0,0) - alpha * Vy(0,0,0);
    sy_p = Ry(0, pc_iy_Ey, pc_iz_Ey) 
            - alpha * Vy(0, pc_iy_Ey, pc_iz_Ey);
    Ey_p = bloch_phaseYZ_y * pc_yz_factor[1] * (sy_p) * 
            (sqrt_sx0(_X) * sqrt_sy1(_Y) * sqrt_sz0(_Z));

    // Ez_p = Rz(0,0,0) - alpha * Vz(0,0,0);
    sz_p = Rz(0, pc_iy_Ez, pc_iz_Ez) 
            - alpha * Vz(0, pc_iy_Ez, pc_iz_Ez);
    Ez_p = bloch_phaseYZ_z * pc_yz_factor[2] * (sz_p) * 
...
for (; _X < _x_end ; _X += _txx) {
    ...
    if (adj_dims) {
        sx = Rx(pc_ix_Ex,pc_iy_Ex,pc_iz_Ex) - alpha * Vx(pc_ix_Ex,pc_iy_Ex,pc_iz_Ex);
        Ex_0[0] = bloch_phaseYZ_x * pc_x_factor[0] * pc_yz_factor[0] *
                    (sx) * (sqrt_sx1(_X) * sqrt_sy0(_Y) * sqrt_sz0(_Z));

        sy_p = Ry(pc_ix_Ey,pc_iy_Ey,pc_iz_Ey) - alpha * Vy(pc_ix_Ey,pc_iy_Ey,pc_iz_Ey);
        Ey_p = bloch_phaseX_y * bloch_phaseYZ_y * pc_x_factor[1] * pc_yz_factor[1] *
                    (sy_p) * (sqrt_sx0(_X+xp) * sqrt_sy1(_Y) * sqrt_sz0(_Z));

        sz_p = Rz(pc_ix_Ez,pc_iy_Ez,pc_iz_Ez) - alpha * Vz(pc_ix_Ez,pc_iy_Ez,pc_iz_Ez);
        Ez_p = bloch_phaseX_z * bloch_phaseYZ_z * pc_x_factor[2] * pc_yz_factor[2] *
                    (sz_p) * (sqrt_sx0(_X+xp) * sqrt_sy0(_Y) * sqrt_sz1(_Z));
    }
