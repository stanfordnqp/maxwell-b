...
if (adj_dims) {
    // Load in s = r - alpha * v.
    Ex_0[0] = bloch_phaseX_x*bloch_phaseYZ_x*(Rx(-1,0,0) - alpha * Vx(-1,0,0)) * 
                (sqrt_sx1(_X+xn) * sqrt_sy0(_Y) * sqrt_sz0(_Z));
    Ey_0[0] = bloch_phaseX_y*bloch_phaseYZ_y*(Ry(-1,0,0) - alpha * Vy(-1,0,0)) * 
                (sqrt_sx0(_X+xn) * sqrt_sy1(_Y) * sqrt_sz0(_Z));
    Ez_0[0] = bloch_phaseX_z*bloch_phaseYZ_z*(Rz(-1,0,0) - alpha * Vz(-1,0,0)) * 
                (sqrt_sx0(_X+xn) * sqrt_sy0(_Y) * sqrt_sz1(_Z));

    // Ey_p = Ry(0,0,0) - alpha * Vy(0,0,0);
    sy_p = Ry(0,0,0) - alpha * Vy(0,0,0);
    Ey_p = bloch_phaseYZ_y * (sy_p) * (sqrt_sx0(_X) * sqrt_sy1(_Y) * sqrt_sz0(_Z));

    // Ez_p = Rz(0,0,0) - alpha * Vz(0,0,0);
    sz_p = Rz(0,0,0) - alpha * Vz(0,0,0);
    Ez_p = bloch_phaseYZ_z * (sz_p) * (sqrt_sx0(_X) * sqrt_sy0(_Y) * sqrt_sz1(_Z));
}
__syncthreads();
...
for (; _X < _x_end ; _X += _txx) {
    ...
    if (adj_dims) {
        sx = Rx(0,0,0) - alpha * Vx(0,0,0);
        Ex_0[0] = bloch_phaseYZ_x * (sx) * (sqrt_sx1(_X) * sqrt_sy0(_Y) * sqrt_sz0(_Z));

        sy_p = Ry(+1,0,0) - alpha * Vy(+1,0,0);    
        Ey_p = bloch_phaseX_y * bloch_phaseYZ_y * (sy_p) * (sqrt_sx0(_X+xp) * sqrt_sy1(_Y) * sqrt_sz0(_Z));

        sz_p = Rz(+1,0,0) - alpha * Vz(+1,0,0);
        Ez_p = bloch_phaseX_z * bloch_phaseYZ_z * (sz_p) * (sqrt_sx0(_X+xp) * sqrt_sy0(_Y) * sqrt_sz1(_Z));
    }
    ...
    if (_in_global && _in_local) {
        ...
        tx = ((1.0 / (sqrt_sx1(_X) * sqrt_sy0(_Y) * sqrt_sz0(_Z))) *
                    (sy0(_Y) * (Hz_0[0] - Hz_0[s_yn])
                    - sz0(_Z) * (Hy_0[0] - Hy_0[s_zn])
                    - ex(0,0,0) * Ex_0[0]));
        ty = ((1.0 / (sqrt_sx0(_X) * sqrt_sy1(_Y) * sqrt_sz0(_Z))) *
                    (sz0(_Z) * (Hx_0[0] - Hx_0[s_zn]) 
                    - sx0(_X) * (Hz_0[0] - Hz_n) 
                    - ey(0,0,0) * Ey_0[0]));
        tz = ((1.0 / (sqrt_sx0(_X) * sqrt_sy0(_Y) * sqrt_sz1(_Z))) *
                    (sx0(_X) * (Hy_0[0] - Hy_n) 
        ...
        omega_num += conj(tx) * sx + conj(ty) * sy + conj(tz) * sz;
        omega_denom += (real(tx) * real(tx)) + (imag(tx) * imag(tx)) + 
	               (real(ty) * real(ty)) + (imag(ty) * imag(ty)) +
		       (real(tz) * real(tz)) + (imag(tz) * imag(tz));
        //omega_num += tx * sx + ty * sy + tz * sz;
        //omega_denom += tx * tx + ty * ty + tz * tz;
